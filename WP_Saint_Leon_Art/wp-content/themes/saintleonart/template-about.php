<?php
/*
Template Name: Page à propos    
*/
get_header();
?>
<?php 
    $about = new WP_Query();
    $about->query([
        'post_type' => 'page',
        'meta_query' => [ 
                [
                    'key'   => '_wp_page_template', 
                    'value' => 'template-about.php'
                ]
            ],
        ]);
;?>
<div itemscope itemtype="http://schema.org/Event">
    <?php if ( $about->have_posts() ): while( $about->have_posts() ): $about->the_post();?>
    <?php $fields = get_fields();?>
    <section class="introduction about" style="background-image: url(<?= dw_asset('images/home.jpg');?>);">
        <div class="introduction__container">
            <h2 class="sla"><span itemprop="name">SaintLeon'Art</span> c'est...</h2>
            <p class="introduction__tagline u-padding-horizontal-small u-11/18@tablet" itemprop="description">
                <?= $fields['tagline']; ?>
            </p>
        </div>
        <?php if( have_rows('button') ): while ( have_rows('button') ) : the_row(); ?>
        <a title="Vers la page notre programme" href="<?php the_sub_field('btn_link'); ?>" class="btn btn--anim" data-text="<?php the_sub_field('btn_name'); ?>"><?php the_sub_field('btn_name'); ?></a>
        <?php endwhile; endif; ?>
    </section>
    <?php if( have_rows('score') ): while ( have_rows('score') ) : the_row(); ?>
        <section class="score">
            <ul>
                <li>
                    <h2>En 2013..</h2>
                </li>
            </ul>
            <div class="score__flex flex">
                <div class="flex__item">
                    <svg version="1.1" id="artist" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 475.605 475.605" style="enable-background:new 0 0 475.605 475.605;">
                        <g>
                            <path d="M448.551,460.605h-39.668V342.613c1.118-0.435,2.15-1.136,2.982-2.112c20.382-23.922,4.84-42.2-1.796-50.005
                                c-1.038-1.22-2.214-2.604-2.594-3.244c-1.853-3.119-1.99-10.1-1.498-14.045c0.397-3.067-1.131-6.064-3.846-7.545
                                c-2.717-1.482-6.063-1.144-8.427,0.851c-10.364,8.74-16.029,19.22-16.837,31.149c-1.541,22.765,15.699,42.116,16.433,42.928
                                c0.182,0.201,0.386,0.367,0.583,0.543v119.472h-20.728v-15.368c0-35.193-9.385-62.355-27.894-80.732
                                c-23.593-23.425-53.203-23.475-54.407-23.474h-37.571v-23.937c24-9.945,39.175-26.501,48.42-40.857
                                c14.433-22.41,17.689-43.977,17.82-44.884c0.052-0.357,0.078-0.716,0.078-1.076v-1.606c5.268-1.299,9.851-4.017,13.49-8.046
                                c10.619-11.757,10.002-30.306,9.245-37.735c-0.578-5.654-3.136-10.399-7.399-13.722c-1.145-0.892-2.363-1.64-3.62-2.268
                                c7.026-1.045,13.064-3.688,18.053-7.917c13.382-11.344,13.375-29.714,13.368-30.424c0-15.534-6.291-30.941-18.698-45.794
                                c-10.454-12.514-24.99-24.366-43.206-35.226c-10.262-6.118-21.404-11.717-32.799-16.589c1.736-7.367,1.197-21.38-11.944-29.77
                                c-3.489-2.229-8.127-1.207-10.357,2.285c-2.229,3.492-1.206,8.129,2.285,10.357c5.274,3.368,5.919,8.359,5.723,11.447
                                c-19.019-6.989-38.029-11.732-54.218-13.235C147.897,7.24,109.013,24.278,91.405,40.327c-8.546,7.789-13.063,15.987-13.063,23.706
                                c0,16.363,10.924,25.823,18.45,30.402c-1.99,5.613-4.557,14.246-5.755,24.099c-2.255,18.536,1.215,34.29,10.028,46.285
                                c-3.241,0.785-6.557,2.122-9.414,4.348c-4.263,3.323-6.821,8.067-7.398,13.723c-0.757,7.429-1.373,25.978,9.245,37.734
                                c3.68,4.074,8.326,6.804,13.667,8.085v1.567c0,0.36,0.026,0.72,0.078,1.076c0.131,0.907,3.388,22.474,17.82,44.884
                                c9.245,14.356,24.419,30.912,48.419,40.857v23.937h-37.518c-1.219,0.008-30.869,0.051-54.461,23.474
                                c-18.509,18.377-27.894,45.54-27.894,80.732v15.368H27.054c-4.142,0-7.5,3.358-7.5,7.5c0,4.142,3.358,7.5,7.5,7.5h421.497
                                c4.142,0,7.5-3.358,7.5-7.5C456.051,463.963,452.693,460.605,448.551,460.605z M391.837,298.606
                                c0.16-2.282,0.597-4.473,1.315-6.585c0.407,1.001,0.879,1.971,1.426,2.891c1.028,1.73,2.435,3.385,4.064,5.301
                                c7.589,8.925,12.358,16.305,3.879,27.94C398.78,323.188,390.954,311.177,391.837,298.606z M321.958,210.571
                                c-0.721,0.798-1.509,1.456-2.358,2.008v-33.596c2.325,0.268,4.651,0.866,6.132,2.029c0.717,0.563,1.477,1.408,1.68,3.4
                                C328.304,193.152,327.339,204.613,321.958,210.571z M101.509,51.413c22.986-20.95,65.093-27.372,96.626-24.444
                                c27.562,2.56,65.747,16.004,95.016,33.454c20.386,12.154,54.586,37.167,54.586,68.162c0,0.122-0.118,12.292-8.144,19.02
                                c-4.649,3.897-11.366,5.425-19.993,4.57v-12.531c3.371,2.267,7.952,1.445,10.309-1.891c2.389-3.384,1.583-8.063-1.8-10.453
                                c-27.134-19.163-53.712-31.635-78.184-39.628c-0.663-0.334-1.383-0.57-2.141-0.695c-29.058-9.228-55.044-12.153-75.288-12.46
                                c-33.75-0.514-57.729,5.805-65.824,8.298c-4.285-2.164-13.33-8.01-13.33-18.78C93.341,61.532,95.487,56.902,101.509,51.413z
                                M109.261,142.951c-0.671,0.573-1.208,1.243-1.625,1.967c-5.829-18.499,1.252-40.221,4.238-47.993
                                c7.632-2.253,29.127-7.66,58.773-7.423c-27.061,3.519-37.678,9.418-39.202,10.341c-2.245,1.359-3.616,3.792-3.616,6.416v20.843
                                L109.261,142.951z M104.629,210.571c-5.381-5.958-6.346-17.419-5.454-26.159c0.203-1.993,0.963-2.837,1.68-3.4
                                c1.518-1.193,3.927-1.793,6.31-2.05v33.729C106.246,212.117,105.4,211.424,104.629,210.571z M137.972,268.577
                                c-11.735-18.011-15.267-35.842-15.808-38.914v-78.004l18.034-15.394c1.669-1.425,2.631-3.51,2.631-5.705v-19.458
                                c2.694-1.027,6.956-2.42,13.097-3.803c10.998-2.478,30.435-5.431,60.576-5.431h28.567c18.76,6.112,38.864,15.024,59.531,27.849
                                v99.944c-0.548,3.107-4.083,20.921-15.807,38.915c-16.809,25.798-41.947,39.985-74.714,42.169h-1.393
                                C179.919,308.562,154.781,294.375,137.972,268.577z M180.982,356.031c4.142,0,7.5-3.358,7.5-7.5v-26.469
                                c7.174,1.836,14.974,3.118,23.473,3.668c0.162,0.01,0.323,0.016,0.485,0.016h1.885c0.162,0,0.323-0.005,0.485-0.016
                                c8.499-0.551,16.299-1.832,23.473-3.668v26.469c0,4.142,3.358,7.5,7.5,7.5h28.056c-2.945,30.767-28.931,54.91-60.456,54.91
                                c-11.754,0-23.155-3.36-32.97-9.717c-5.105-3.306-10.89-0.719-17.016,2.272c0.046-0.188,0.092-0.371,0.135-0.544
                                c1.776-7.148,3.31-13.321,0.099-17.895c-6.068-8.644-9.711-18.582-10.703-29.026H180.982z M68.61,445.237
                                c0-88.177,64.527-89.206,67.301-89.206h1.953c0.99,13.033,5.292,25.476,12.605,36.358c-0.192,1.701-0.988,4.905-1.495,6.946
                                c-2.005,8.067-3.736,15.035,1.086,19.642c5.366,5.126,12.674,1.545,19.121-1.612c1.433-0.702,3.591-1.759,5.039-2.321
                                c11.794,7.136,25.284,10.897,39.162,10.897c39.8,0,72.505-30.864,75.496-69.91h2.03c0.002,0,0.005,0,0.009,0
                                c0.59,0,24.707,0.186,43.775,19.118c15.568,15.457,23.462,39.039,23.462,70.088v15.368H68.61V445.237z"/>
                            <circle cx="160.146" cy="167.928" r="8.952"/>
                            <circle cx="266.62" cy="167.928" r="8.951"/>
                            <path d="M211.448,220.656h3.868c9.925,0,18-8.075,18-18c0-4.142-3.358-7.5-7.5-7.5c-4.142,0-7.5,3.358-7.5,7.5c0,1.654-1.346,3-3,3
                                h-3.868c-1.654,0-3-1.346-3-3c0-4.142-3.358-7.5-7.5-7.5c-4.142,0-7.5,3.358-7.5,7.5
                                C193.448,212.581,201.523,220.656,211.448,220.656z"/>
                            <path d="M259.877,260.668c2.601-3.225,2.095-7.946-1.129-10.546c-3.226-2.601-7.947-2.094-10.546,1.129
                                c-0.08,0.1-9.089,9.939-32.971,9.939c-24.809,0-36.903-10.295-37.494-10.812c-3.061-2.749-7.773-2.519-10.552,0.528
                                c-2.791,3.061-2.572,7.805,0.489,10.595c0.658,0.6,16.468,14.689,47.557,14.689C246.456,276.191,258.595,262.257,259.877,260.668z"
                                />
                        </g>
                    </svg>
                    <span class="count"><?php the_sub_field('score_artist'); ?></span>
                    <span class="text">Artistes</span>
                </div>
                <div class="flex__item">
                    <svg version="1.1" id="ticket" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 512.001 512.001" style="enable-background:new 0 0 512.001 512.001;">
                        <g>
                            <g>
                                <path d="M504.422,266.124h-9.793c-8.204,0-14.878-6.674-14.878-14.878s6.673-14.878,14.877-14.878h9.793
                                    c4.187,0,7.579-3.392,7.579-7.579v-45.47c0-4.187-3.392-7.579-7.579-7.579h-9.793c-8.204,0-14.878-6.674-14.878-14.878v-2.756
                                    l11.117-10.183c3.014-2.761,3.296-7.415,0.637-10.52l-29.585-34.529c-1.308-1.527-3.169-2.471-5.173-2.625
                                    c-2.008-0.168-3.989,0.494-5.513,1.801l-7.437,6.373c-6.23,5.337-15.642,4.611-20.979-1.618
                                    c-2.586-3.017-3.842-6.862-3.536-10.825c0.305-3.962,2.135-7.568,5.154-10.154l7.437-6.373c3.179-2.723,3.548-7.508,0.824-10.686
                                    L403.11,34.239c-2.722-3.177-7.507-3.548-10.686-0.824l-7.438,6.372c-3.018,2.586-6.861,3.844-10.824,3.536
                                    c-3.962-0.305-7.568-2.135-10.154-5.154l-9.922-11.581c-1.308-1.527-3.169-2.471-5.173-2.625
                                    c-2.011-0.154-3.988,0.492-5.513,1.801L212.373,138.032h-21.635c-4.187,0-7.579,3.392-7.579,7.579
                                    c0,4.187,3.392,7.579,7.579,7.579h273.855v7.672c0,16.562,13.474,30.036,30.036,30.036h2.214v30.312h-2.214
                                    c-16.562,0-30.036,13.474-30.036,30.036s13.474,30.036,30.036,30.036h2.214v30.312h-2.214c-16.562,0-30.036,13.473-30.036,30.035
                                    v17.182h-52.404c-4.187,0-7.579,3.392-7.579,7.579s3.392,7.579,7.579,7.579h59.983c4.187,0,7.579-3.392,7.579-7.579v-24.761
                                    c0-8.203,6.674-14.877,14.878-14.877h9.793c4.187,0,7.579-3.392,7.579-7.579v-45.47
                                    C512.001,269.516,508.609,266.124,504.422,266.124z M235.67,138.032l111.836-95.824l4.991,5.826
                                    c5.219,6.093,12.5,9.788,20.499,10.404c7.998,0.615,15.759-1.918,21.852-7.138l1.683-1.441l19.723,23.017l-1.682,1.442
                                    c-12.576,10.775-14.042,29.774-3.265,42.351c10.777,12.576,29.776,14.041,42.351,3.265l1.682-1.442l16.743,19.54H235.67z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M345.666,358.81H47.409v-7.671c0-16.562-13.474-30.036-30.036-30.036h-2.215v-30.312h2.215
                                    c16.562,0,30.036-13.474,30.036-30.036s-13.474-30.036-30.036-30.036h-2.215v-30.312h2.215c16.562,0,30.036-13.474,30.036-30.036
                                    v-17.182h86.493c4.187,0,7.579-3.392,7.579-7.579s-3.392-7.579-7.579-7.579H39.83c-4.187,0-7.579,3.392-7.579,7.579v24.761
                                    c0,8.203-6.674,14.878-14.878,14.878H7.579c-4.187,0-7.579,3.392-7.579,7.579v45.47c0,4.187,3.392,7.579,7.579,7.579h9.794
                                    c8.203,0,14.878,6.674,14.878,14.878c0,8.204-6.674,14.878-14.878,14.878H7.579c-4.187,0-7.579,3.392-7.579,7.579v45.47
                                    c0,4.187,3.392,7.579,7.579,7.579h9.794c8.203,0,14.878,6.674,14.878,14.878v4.098l-10.708,8.492
                                    c-1.615,1.28-2.638,3.162-2.836,5.211c-0.197,2.05,0.449,4.094,1.79,5.658l29.584,34.529c1.308,1.527,3.169,2.471,5.173,2.625
                                    c2.004,0.167,3.988-0.493,5.513-1.801l7.438-6.373c3.018-2.587,6.862-3.851,10.824-3.536c3.962,0.305,7.568,2.135,10.154,5.154
                                    c5.338,6.23,4.612,15.641-1.618,20.979l-7.437,6.372c-3.179,2.723-3.548,7.508-0.824,10.686l29.584,34.529
                                    c2.723,3.179,7.508,3.55,10.686,0.824l7.438-6.372c3.017-2.586,6.873-3.845,10.824-3.536c3.962,0.305,7.568,2.135,10.154,5.154
                                    l9.922,11.58c1.499,1.749,3.623,2.648,5.759,2.648c1.745,0,3.498-0.599,4.927-1.824l130.796-112.069
                                    c0.073-0.063,0.136-0.131,0.207-0.196h46.061c4.187,0,7.579-3.392,7.579-7.579S349.853,358.81,345.666,358.81z M164.495,469.792
                                    l-4.991-5.825c-5.219-6.093-12.499-9.788-20.499-10.404c-7.999-0.621-15.759,1.918-21.852,7.138l-1.683,1.441l-19.723-23.018
                                    l1.682-1.441c6.092-5.22,9.788-12.5,10.404-20.5c0.616-7.999-1.918-15.76-7.138-21.852c-5.219-6.093-12.499-9.788-20.499-10.404
                                    c-7.997-0.607-15.759,1.919-21.852,7.138l-1.682,1.442l-16.744-19.54H276.33L164.495,469.792z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M427.899,173.562H212.007c-4.187,0-7.579,3.392-7.579,7.579s3.392,7.579,7.579,7.579H420.32v134.56H91.681V188.72h62.402
                                    c4.187,0,7.579-3.392,7.579-7.579s-3.392-7.579-7.579-7.579H84.102c-4.187,0-7.579,3.393-7.579,7.579v149.718
                                    c0,4.187,3.392,7.579,7.579,7.579H427.9c4.187,0,7.579-3.392,7.579-7.579V181.141
                                    C435.478,176.954,432.086,173.562,427.899,173.562z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M156.673,222.827h-41.854c-2.771,0-4.057,2.969-4.057,5.739c0,3.166,1.484,5.838,4.057,5.838h14.546V291.1
                                    c0,2.672,3.167,4.057,6.431,4.057c3.167,0,6.432-1.385,6.432-4.057v-56.696h14.446c2.573,0,4.057-2.771,4.057-5.838
                                    C160.73,225.796,159.444,222.827,156.673,222.827z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M174.186,222.827c-3.265,0-6.431,1.187-6.431,3.957V291.1c0,2.672,3.166,4.057,6.431,4.057
                                    c3.167,0,6.431-1.385,6.431-4.057v-64.316C180.617,224.014,177.353,222.827,174.186,222.827z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M231.675,269.826c-3.364,0-6.135,0.99-6.333,4.057c-0.297,4.156-1.088,10.587-10.092,10.587
                                    c-6.827,0-10.587-3.661-10.587-11.379v-27.606c0-7.718,3.759-11.38,10.389-11.38c9.399,0,10.092,6.729,10.291,9.796
                                    c0.098,2.969,2.771,4.057,6.431,4.057c4.353,0,6.432-1.187,6.432-6.333c0-11.479-9.797-18.8-23.551-18.8
                                    c-12.467,0-22.856,6.134-22.856,22.659v27.606c0,16.524,10.291,22.659,22.758,22.659c13.852,0,23.649-7.619,23.649-19.592
                                    C238.206,271.111,236.127,269.826,231.675,269.826z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M295.195,286.945l-21.571-32.85l17.711-22.165c0.692-0.89,0.791-1.781,0.791-2.473c0-3.266-4.156-6.63-7.421-6.63
                                    c-1.384,0-2.572,0.494-3.561,1.88l-21.373,29.09v-27.013c0-2.77-3.264-3.957-6.431-3.957c-3.265,0-6.431,1.187-6.431,3.957h0.001
                                    V291.1c0,2.672,3.166,4.057,6.431,4.057c3.167,0,6.431-1.385,6.431-4.057v-19.393l5.344-6.728l18.602,28.2
                                    c1.088,1.78,2.77,2.473,4.452,2.473c3.76,0,7.717-3.264,7.717-6.431C295.887,288.43,295.689,287.638,295.195,286.945z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M342.39,283.878h-28.003v-19.889h15.041c2.771,0,4.057-2.672,4.057-4.948c0-2.672-1.484-5.146-4.057-5.146h-15.041
                                    v-19.789h28.003c2.573,0,4.057-2.672,4.057-5.739c0-2.671-1.287-5.541-4.057-5.541h-35.225c-2.87,0-5.64,1.385-5.64,4.057V291.1
                                    c0,2.672,2.77,4.057,5.64,4.057h35.225c2.771,0,4.057-2.869,4.057-5.541C346.447,286.55,344.963,283.878,342.39,283.878z"/>
                            </g>
                        </g>
                        <g>
                            <g>
                                <path d="M395.127,222.827h-41.854c-2.771,0-4.057,2.969-4.057,5.739c0,3.166,1.484,5.838,4.057,5.838h14.546V291.1
                                    c0,2.672,3.167,4.057,6.431,4.057c3.167,0,6.432-1.385,6.432-4.057v-56.696h14.446c2.573,0,4.057-2.771,4.057-5.838
                                    C399.184,225.796,397.898,222.827,395.127,222.827z"/>
                            </g>
                        </g>
                    </svg>
                    <span class="count"><?php the_sub_field('score_visit'); ?></span>
                    <span class="text">Visiteurs</span>
                </div>
                <div class="flex__item">
                    <svg version="1.1" id="artwork" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="210.326px" height="210.326px" viewBox="0 0 210.326 210.326" style="enable-background:new 0 0 210.326 210.326;">
                        <g>
                            <g>
                                <path d="M14.101,100.29c1.398,0.754,3.136,0.477,4.223-0.683l31.318-33.322l28.161,37.411c0.659,0.876,1.692,1.391,2.789,1.395
                                    c0.003,0,0.003,0,0.007,0c1.097,0,2.127-0.512,2.789-1.384l13.947-18.352l31.359,38.592c0.66,0.814,1.663,1.294,2.717,1.294h69.21
                                    c1.067,0,2.081-0.493,2.748-1.328c0.66-0.839,0.91-1.937,0.667-2.978l-19.711-84.104c-0.373-1.584-1.787-2.702-3.416-2.702
                                    h-59.135V18.798c0-1.938-1.57-3.504-3.504-3.504H91.988c-1.936,0-3.504,1.565-3.504,3.504v15.332H30.225
                                    c-1.615,0-3.025,1.104-3.407,2.681L12.362,96.383C11.988,97.929,12.703,99.534,14.101,100.29z M95.492,22.302h19.273v11.827
                                    H95.492V22.302z M32.979,41.138h59.009h26.282h59.86l18.069,77.095h-63.119L99.963,77.475c-0.676-0.832-1.671-1.392-2.775-1.294
                                    c-1.076,0.017-2.081,0.525-2.732,1.384L80.612,95.776L52.735,58.743c-0.62-0.823-1.576-1.335-2.606-1.388
                                    c-1.042-0.031-2.036,0.347-2.745,1.097L22.304,85.138L32.979,41.138z"/>
                                <path d="M121.555,80.123h9.938c0.309,1.153,0.767,2.243,1.352,3.253l-7.032,7.029c-1.368,1.371-1.368,3.585,0,4.956
                                    c0.685,0.683,1.581,1.026,2.478,1.026s1.797-0.344,2.478-1.026l7.036-7.031c1.014,0.585,2.095,1.045,3.248,1.353v9.935
                                    c0,1.939,1.57,3.504,3.504,3.504s3.505-1.565,3.505-3.504v-9.935c1.153-0.311,2.241-0.768,3.247-1.353l7.029,7.031
                                    c0.685,0.683,1.581,1.026,2.478,1.026s1.793-0.344,2.478-1.026c1.369-1.371,1.369-3.585,0-4.956l-7.029-7.029
                                    c0.586-1.013,1.048-2.1,1.355-3.253h9.932c1.937,0,3.504-1.565,3.504-3.504c0-1.938-1.567-3.504-3.504-3.504h-9.932
                                    c-0.308-1.153-0.77-2.24-1.355-3.252l7.029-7.026c1.369-1.369,1.369-3.584,0-4.955c-1.365-1.369-3.583-1.369-4.955,0l-7.029,7.027
                                    c-1.013-0.587-2.098-1.042-3.251-1.354V53.62c0-1.938-1.57-3.504-3.504-3.504c-1.938,0-3.505,1.566-3.505,3.504v9.935
                                    c-1.153,0.308-2.238,0.767-3.251,1.354l-7.022-7.031c-1.368-1.369-3.586-1.369-4.955,0c-1.369,1.367-1.369,3.584,0,4.955
                                    l7.022,7.033c-0.589,1.009-1.04,2.096-1.355,3.249h-9.931c-1.938,0-3.504,1.566-3.504,3.504
                                    C118.051,78.558,119.618,80.123,121.555,80.123z M144.552,70.048c3.624,0,6.57,2.946,6.57,6.57s-2.946,6.571-6.57,6.571
                                    s-6.571-2.947-6.571-6.571S140.928,70.048,144.552,70.048z"/>
                                <path d="M209.689,189.513l-15.05-33.609h12.114c1.934,0,3.504-1.567,3.504-3.504v-12.266c0-1.937-1.57-3.504-3.504-3.504H3.504
                                    c-1.936,0-3.504,1.567-3.504,3.504v12.266c0,1.937,1.569,3.504,3.504,3.504h13.866L2.057,190.098
                                    c-0.488,1.081-0.388,2.337,0.257,3.337c0.646,0.995,1.752,1.598,2.939,1.598h19.273c1.381,0,2.632-0.808,3.2-2.073l16.593-37.056
                                    h123.367l16.594,37.056c0.564,1.259,1.817,2.073,3.199,2.073h19.274c0.021-0.003,0.048-0.003,0.068,0
                                    c1.937,0,3.504-1.567,3.504-3.504C210.326,190.782,210.093,190.084,209.689,189.513z M7.008,143.639h196.24v5.256h-14.017h-19.273
                                    H42.051H22.778H7.008V143.639z M22.259,188.027H10.664l14.381-32.12h11.596L22.259,188.027z M189.751,188.027l-14.383-32.12
                                    h11.598l14.38,32.12H189.751z"/>
                            </g>
                        </g>
                    </svg>
                    <span class="count"><?php the_sub_field('score_sold'); ?></span>
                    <span class="text">Oeuvres vendues</span>
                </div>
            </div>
        </section>
    <?php endwhile; endif; ?>

    <?php if (have_rows('gallery')):?>
    <section class="gallery">
        <h2 class="u-margin-bottom">En images</h2>
        <div id="Glide" class="glide">
            <div class="glide__arrows">
                <button class="glide__arrow prev" data-glide-dir="<">prev</button>
                <button class="glide__arrow next" data-glide-dir=">">next</button>
            </div>
        
            <div class="glide__wrapper">
                <div class="glide__track" itemscope itemtype="http://schema.org/Event">
                    <?php $images = get_field('gallery'); if($images): ?>
                    <?php foreach ($images as $image) : ?>
                        <img itemprop="image" class="glide__slide" src="<?= $image['gallery_item']['url']; ?>" alt="<?php echo $image['gallery_item']['alt']; ?>">
                    <?php endforeach; ?>
                <?php endif; ?>
                </div>
            </div>
            <div class="glide__bullets"></div>
        </div>
        
        <div class="gallery__cta u-margin-top">
            <a title="Vers la gallerie de notre page facebook" href="http://instagram.be" class="btn btn--anim" data-text="Notre galerie instagram">Notre galerie instagram</a>
            <a title="Vers la gallerie de notre page instagram" href="http://facebook.be" class="btn btn--anim" data-text="Notre galerie facebook">Notre galerie facebook</a>
            <a title="Vers la gallerie de notre page twitter" href="http://twitter.be" class="btn btn--anim" data-text="Notre galerie twitter">Notre galerie twitter</a>
            <div class="btn--mobile">
                <h3 class="u-margin-bottom">Nos galeries</h3>
                <a title="Vers notre page facebook" href="http://facebook.be">
                    facebook
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96.124 96.123">
                        <path d="M72.089.02L59.624 0C45.62 0 36.57 9.285 36.57 23.656v10.907H24.037a1.96 1.96 0 0 0-1.96 1.961v15.803a1.96 1.96 0 0 0 1.96 1.96H36.57v39.876a1.96 1.96 0 0 0 1.96 1.96h16.352a1.96 1.96 0 0 0 1.96-1.96V54.287h14.654a1.96 1.96 0 0 0 1.96-1.96l.006-15.803a1.963 1.963 0 0 0-1.961-1.961H56.842v-9.246c0-4.444 1.059-6.7 6.848-6.7l8.397-.003a1.96 1.96 0 0 0 1.959-1.96V1.98A1.96 1.96 0 0 0 72.089.02z"/>
                    </svg>
                </a>
                <a title="Vers notre page instagram" href="http://instagram.be">
                    instagram
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 169.063 169.063">
                        <path d="M122.406 0H46.654C20.929 0 0 20.93 0 46.655v75.752c0 25.726 20.929 46.655 46.654 46.655h75.752c25.727 0 46.656-20.93 46.656-46.655V46.655C169.063 20.93 148.133 0 122.406 0zm31.657 122.407c0 17.455-14.201 31.655-31.656 31.655H46.654C29.2 154.063 15 139.862 15 122.407V46.655C15 29.201 29.2 15 46.654 15h75.752c17.455 0 31.656 14.201 31.656 31.655v75.752z"/>
                        <path d="M84.531 40.97c-24.021 0-43.563 19.542-43.563 43.563 0 24.02 19.542 43.561 43.563 43.561s43.563-19.541 43.563-43.561c0-24.021-19.542-43.563-43.563-43.563zm0 72.123c-15.749 0-28.563-12.812-28.563-28.561 0-15.75 12.813-28.563 28.563-28.563s28.563 12.813 28.563 28.563c0 15.749-12.814 28.561-28.563 28.561zM129.921 28.251c-2.89 0-5.729 1.17-7.77 3.22a11.053 11.053 0 0 0-3.23 7.78c0 2.891 1.18 5.73 3.23 7.78 2.04 2.04 4.88 3.22 7.77 3.22 2.9 0 5.73-1.18 7.78-3.22 2.05-2.05 3.22-4.89 3.22-7.78 0-2.9-1.17-5.74-3.22-7.78-2.04-2.05-4.88-3.22-7.78-3.22z"/>
                    </svg>
                </a>
                <a title="Vers notre page twitter" href="http://twitter.be">
                    twitter
                    <svg version="1.1"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 612 612" style="enable-background:new 0 0 612 612;">
                        <g>
                            <g>
                                <path d="M612,116.258c-22.525,9.981-46.694,16.75-72.088,19.772c25.929-15.527,45.777-40.155,55.184-69.411
                                    c-24.322,14.379-51.169,24.82-79.775,30.48c-22.907-24.437-55.49-39.658-91.63-39.658c-69.334,0-125.551,56.217-125.551,125.513
                                    c0,9.828,1.109,19.427,3.251,28.606C197.065,206.32,104.556,156.337,42.641,80.386c-10.823,18.51-16.98,40.078-16.98,63.101
                                    c0,43.559,22.181,81.993,55.835,104.479c-20.575-0.688-39.926-6.348-56.867-15.756v1.568c0,60.806,43.291,111.554,100.693,123.104
                                    c-10.517,2.83-21.607,4.398-33.08,4.398c-8.107,0-15.947-0.803-23.634-2.333c15.985,49.907,62.336,86.199,117.253,87.194
                                    c-42.947,33.654-97.099,53.655-155.916,53.655c-10.134,0-20.116-0.612-29.944-1.721c55.567,35.681,121.536,56.485,192.438,56.485
                                    c230.948,0,357.188-191.291,357.188-357.188l-0.421-16.253C573.872,163.526,595.211,141.422,612,116.258z"/>
                            </g>
                        </g>
                    </svg>
                </a>
            </div>
        </div>
    </section>
    <?php endif; ?>
    <?php if( $fields['asbl_description'] ): ?>
        <section class="asbl">
            <h2 class="u-margin-bottom"><span class="sla">SaintLeon'Art</span><span><?= $fields['asbl_title']; ?></span></h2>
            <div class="flex">
                <p>
                    <?= $fields['asbl_description']; ?>
                </p>
            </div>
            <?php if( have_rows('asbl_button') ): while ( have_rows('asbl_button') ) : the_row(); ?>
            <a title="Vers la page actualités" href="<?php the_sub_field('btn_link'); ?>" class="btn btn--anim" data-text="<?php the_sub_field('btn_name'); ?>"><?php the_sub_field('btn_name'); ?></a>
            <?php endwhile; endif; ?>
        </section>
    <?php endif;?>
    <?php if( have_rows('team') ): ?>
        <section class="team"  >
            <h2>Rencontrez l'équipe</h2>
            <div class="team__container o-flex">
                <?php while ( have_rows('team') ) : the_row(); ?>
                <div class="o-flex__item" itemprop="composer" itemscope itemtype="http://schema.org/Person">
                    <?php sla_image_attribute('photo'); ?>
                    <div class="info">
                        <p class="name" itemprop="givenName"><?php the_sub_field('surname'); ?> <?php the_sub_field('name'); ?></p>
                        <p class="job" itemprop="jobTitle"><?php the_sub_field('job'); ?></p>
                    </div>
                    <div class="social">
                        <a title="Vers la page facebook de <?php the_sub_field('surname') ;?>" itemprop="follows" href="<?php the_sub_field('facebook'); ?>">
                            facebook
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96.124 96.123">
                                <path d="M72.089.02L59.624 0C45.62 0 36.57 9.285 36.57 23.656v10.907H24.037a1.96 1.96 0 0 0-1.96 1.961v15.803a1.96 1.96 0 0 0 1.96 1.96H36.57v39.876a1.96 1.96 0 0 0 1.96 1.96h16.352a1.96 1.96 0 0 0 1.96-1.96V54.287h14.654a1.96 1.96 0 0 0 1.96-1.96l.006-15.803a1.963 1.963 0 0 0-1.961-1.961H56.842v-9.246c0-4.444 1.059-6.7 6.848-6.7l8.397-.003a1.96 1.96 0 0 0 1.959-1.96V1.98A1.96 1.96 0 0 0 72.089.02z"/>
                            </svg>
                        </a>
                        <a title="Vers la page instagram de <?php the_sub_field('surname') ;?>" itemprop="follows" href="<?php the_sub_field('instagram'); ?>">
                            instagram
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 169.063 169.063">
                                <path d="M122.406 0H46.654C20.929 0 0 20.93 0 46.655v75.752c0 25.726 20.929 46.655 46.654 46.655h75.752c25.727 0 46.656-20.93 46.656-46.655V46.655C169.063 20.93 148.133 0 122.406 0zm31.657 122.407c0 17.455-14.201 31.655-31.656 31.655H46.654C29.2 154.063 15 139.862 15 122.407V46.655C15 29.201 29.2 15 46.654 15h75.752c17.455 0 31.656 14.201 31.656 31.655v75.752z"/>
                                <path d="M84.531 40.97c-24.021 0-43.563 19.542-43.563 43.563 0 24.02 19.542 43.561 43.563 43.561s43.563-19.541 43.563-43.561c0-24.021-19.542-43.563-43.563-43.563zm0 72.123c-15.749 0-28.563-12.812-28.563-28.561 0-15.75 12.813-28.563 28.563-28.563s28.563 12.813 28.563 28.563c0 15.749-12.814 28.561-28.563 28.561zM129.921 28.251c-2.89 0-5.729 1.17-7.77 3.22a11.053 11.053 0 0 0-3.23 7.78c0 2.891 1.18 5.73 3.23 7.78 2.04 2.04 4.88 3.22 7.77 3.22 2.9 0 5.73-1.18 7.78-3.22 2.05-2.05 3.22-4.89 3.22-7.78 0-2.9-1.17-5.74-3.22-7.78-2.04-2.05-4.88-3.22-7.78-3.22z"/>
                            </svg>
                        </a>
                        <a title="Vers la page twitter de <?php the_sub_field('surname') ;?>" itemprop="follows" href="<?php the_sub_field('twitter'); ?>">
                            twitter
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 612 612" style="enable-background:new 0 0 612 612;">
                                <g>
                                    <g>
                                        <path d="M612,116.258c-22.525,9.981-46.694,16.75-72.088,19.772c25.929-15.527,45.777-40.155,55.184-69.411
                                            c-24.322,14.379-51.169,24.82-79.775,30.48c-22.907-24.437-55.49-39.658-91.63-39.658c-69.334,0-125.551,56.217-125.551,125.513
                                            c0,9.828,1.109,19.427,3.251,28.606C197.065,206.32,104.556,156.337,42.641,80.386c-10.823,18.51-16.98,40.078-16.98,63.101
                                            c0,43.559,22.181,81.993,55.835,104.479c-20.575-0.688-39.926-6.348-56.867-15.756v1.568c0,60.806,43.291,111.554,100.693,123.104
                                            c-10.517,2.83-21.607,4.398-33.08,4.398c-8.107,0-15.947-0.803-23.634-2.333c15.985,49.907,62.336,86.199,117.253,87.194
                                            c-42.947,33.654-97.099,53.655-155.916,53.655c-10.134,0-20.116-0.612-29.944-1.721c55.567,35.681,121.536,56.485,192.438,56.485
                                            c230.948,0,357.188-191.291,357.188-357.188l-0.421-16.253C573.872,163.526,595.211,141.422,612,116.258z"/>
                                    </g>
                                </g>
                            </svg>
                        </a>
                        <a title="Vers votre boîte email" itemprop="email" href="mailto:<?php the_sub_field('email'); ?>">
                            mail
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 31.012 31.012" style="enable-background:new 0 0 31.012 31.012;">
                                <g>
                                    <path d="M28.512,26.529H2.5c-1.378,0-2.5-1.121-2.5-2.5V6.982c0-1.379,1.122-2.5,2.5-2.5h26.012c1.378,0,2.5,1.121,2.5,2.5v17.047
                                        C31.012,25.408,29.89,26.529,28.512,26.529z M2.5,5.482c-0.827,0-1.5,0.673-1.5,1.5v17.047c0,0.827,0.673,1.5,1.5,1.5h26.012
                                        c0.827,0,1.5-0.673,1.5-1.5V6.982c0-0.827-0.673-1.5-1.5-1.5H2.5z"/>
                                    <path d="M15.506,18.018c-0.665,0-1.33-0.221-1.836-0.662L0.891,6.219c-0.208-0.182-0.23-0.497-0.048-0.705
                                        c0.182-0.21,0.498-0.23,0.706-0.049l12.778,11.137c0.64,0.557,1.72,0.556,2.358,0L29.46,5.466c0.207-0.183,0.522-0.162,0.706,0.049
                                        c0.182,0.208,0.16,0.523-0.048,0.705L17.342,17.355C16.836,17.797,16.171,18.018,15.506,18.018z"/>
                                </g>
                            </svg>
                        </a>
                    </div>
                </div>
                <?php endwhile;?>
            </div>
        </section>
    <?php endif; ?>
    <?php if (have_rows('file_repeater')) : ?>
    <section class="press">
        <h2>Notre presse</h2>
        <div class="press__container container">
            <?php while (have_rows('file_repeater')) : the_row(); ?>
                <?php ms_the_data_file(); ?>
            <?php endwhile;?>
        </div>
        <a title="Vers la page toutes nos actualités" href="" class="btn btn--anim" data-text="Voir nos actualités">Voir nos actualités</a>
    </section>
    <?php endif;?>
    <?php endwhile;endif; ?>
</div>
<?php get_footer(); ?>